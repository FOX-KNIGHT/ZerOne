// shared/ciphers.js
// All 11 ciphers used in ZerOne — encode + decode for each
...
export function hill(text, keyMatrix, decrypt = false) {
  ...
  for (let i = 0; i < padded.length; i += 2) {
    const v0 = padded.charCodeAt(i) - 65;
    const v1 = padded.charCodeAt(i + 1) - 65;
    result +=
      String.fromCharCode(((mat[0][0] * v0 + mat[0][1] * v1) % 26) + 65) +
      String.fromCharCode(((mat[1][0] * v0 + mat[1][1] * v1) % 26) + 65);
  }
  return result;
}

// ─── 7. BEAUFORT ──────────────────────────────────────────────────────────────
// Reciprocal: same function for encrypt and decrypt
...
export function autokey(text, key, decrypt = false) {
  ...
  if (!decrypt) {
    ...
    for (let i = 0; i < t.length; i++) {
      ...
      plain += String.fromCharCode(((t.charCodeAt(i) - 65 - ki + 26) % 26) + 65);
    }
    return plain;
  }
}
...
export function affine(text, a, b, decrypt = false) {
  ...
  if (!decrypt) {
    ...
    return t.replace(/[A-Z]/g, (ch) =>
      String.fromCharCode(((aInv * ((ch.charCodeAt(0) - 65 - b + 26) % 26)) % 26) + 65)
    );
  }
}
...
export function applyCipher(cipherName, text, params, decrypt = false) {
  switch (cipherName.toLowerCase().replace(/[\s_-]/g, '')) {
...
export const CIPHER_LIST = [
  { name: 'Caesar',              params: ['shift (number)'] },
  { name: 'Atbash',              params: [] },
  { name: 'Vigenere',            params: ['key (word)'] },
  { name: 'Playfair',            params: ['key (word)'] },
  { name: 'RowTransposition',    params: ['key (digit sequence e.g. 3142)'] },
  { name: 'Hill',                params: ['matrix [[a,b],[c,d]]'] },
  { name: 'Beaufort',            params: ['key (word)'] },
  { name: 'DoubleTransposition', params: ['key1, key2 (digit sequences)'] },
  { name: 'Autokey',             params: ['key (word)'] },
  { name: 'Affine',              params: ['a (coprime to 26), b (number)'] },
  { name: 'RailFence',           params: ['rails (number ≥ 2)'] },
];
